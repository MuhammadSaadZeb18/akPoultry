...
const ContactSection = () => {
  ...
  return (
    <section ref={sectionRef} id="contact" className="section-padding bg-muted relative overflow-hidden">
      {/* Background decorative elements */}
      <div className="absolute top-20 right-20 w-48 h-48 rounded-full bg-gold/10 blur-2xl animate-float" />
      <div className="absolute bottom-20 left-10 w-64 h-64 rounded-full bg-emerald/10 blur-2xl animate-float" style={{ animationDelay: '2s' }} />

      <div className="section-container relative z-10">
        {/* Section Header */}
        <div className={`text-center mb-16 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
          <span className="badge-gold mb-4 inline-block animate-glow-pulse">Get In Touch</span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-forest mb-4">
            Contact Us
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
            Ready to improve your poultry operation? Let's discuss how we can help.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-6">
            {contactInfo.map((info, index) => (
              <div 
                key={info.title}
                className={`flex items-start gap-4 p-6 glass-enhanced hover-lift-glow cursor-default ${isVisible ? 'animate-slide-in-left' : 'opacity-0'}`}
              ...
              >
                ...
                <div>
                  <h3 className="font-semibold text-forest mb-1">{info.title}</h3>
          ...
          {/* Call to Action Card */}
          <div className={`relative ${isVisible ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: '0.5s', animationFillMode: 'forwards' }}>
            <div 
              ...
              style={{
                background: 'linear-gradient(135deg, hsl(142 55% 18% / 0.9) 0%, hsl(142 60% 12% / 0.95) 100%)'
              }}
            >
